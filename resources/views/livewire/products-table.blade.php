        ...
        @forelse ($products as $product)
            <tr>
                ...
                @if($cart->where('id', $product->id)->count())
                    <form  wire:submit.prevent="deletFromCart({{ $product->id }})" action="{{ route('cart.delete') }}" method="post">
                        @csrf
                        @method('DELETE')
                ...
                <form wire:submit.prevent="addToCart({{ $product->id }})" action="{{ route('cart.store') }}" method="POST">
                        @csrf
                        <input type="hidden" name="product_id" value="{{ $product->id }}">
                        <input wire:model="quantity.{{ $product->id }}" type="number" name="quantity" min="1"
